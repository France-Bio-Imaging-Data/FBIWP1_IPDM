...
public class Interpolator3DV {
private String inPath, outPath, fileName;	
private String[] paths;
private double[] pos;
private double[] interpPos;

public Interpolator3DV(String inpath, String outpath, String set){
	inPath = inpath;
	outPath = outpath;
	fileName = set;
...
public void interpolate3DPsf(int zernikeOrder, int interpOrder,int mapping){
	...
	for (int orint = 1; orint <= 8 ; orint++){
		for (int plan = 0; plan < plans; plan++){
			IJ.log("Function Estimating for plan " + plan + " variations" );
			func.setPlanFit(interp.getInterpolatorFunction(orint, result.getGlobalMVar(plan), getPos()), plan);
		}
		...
		for (int psf = 0; psf < resInt.getPsfNum(); psf++){
			...
			IJ.save(new ImagePlus("res" + String.valueOf(psf),npsf), outPath + File.separator + interpPos[psf] + orint + "res.tif");
		}
	}
}
}